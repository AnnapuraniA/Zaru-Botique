...
function EmailTemplates() {
  ...
  const handleSave = async () => {
    if (!templateForm.name || !templateForm.type || !templateForm.body) {
    ...
    if (templateForm.channel === 'email' && !templateForm.subject) {
      showError('Subject is required for email templates')
      return
    }
    ...
    try {
      ...
      setTemplateForm({ name: '', type: '', channel: 'email', subject: '', body: '', variables: [] })
      await loadTemplates()
    } catch (err) {
  ...
  const handleEdit = async (template) => {
    try {
      ...
      setTemplateForm({
        ...
        channel: fullTemplate.channel || 'email',
  ...
  const filteredTemplates = templates.filter(template =>
    (!channelFilter || template.channel === channelFilter) &&
    (!searchQuery || 
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1>SMS/Email Templates</h1>
          <p>Manage SMS and email templates for order confirmations, notifications, and more</p>
        </div>
        <button 
            ...
            <div key={template.id} className="template-card">
              <div className="template-header">
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <span className="template-channel">{template.channel || 'email'}</span>
                  <button 
                    className="btn-icon" 
                    title="Edit"
              ...
              <div className="template-content">
                ...
                <div className="template-body-preview">
                  {template.body?.substring(0, template.channel === 'sms' ? 80 : 150)}
                  {template.body?.length > (template.channel === 'sms' ? 80 : 150) && '...'}
                </div>
              </div>
            </div>
          ))
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <label>{templateForm.channel === 'email' ? 'Email' : 'SMS'} Body *</label>
                  {templateForm.type && (
                    <button
                      type="button"
                      className="btn btn-outline"
                ...
                <textarea
                  ...
                  rows={templateForm.channel === 'sms' ? 5 : 15}
                  value={templateForm.body}
                  onChange={(e) => setTemplateForm({ ...templateForm, body: e.target.value })}
                  placeholder={templateForm.channel === 'sms' 
                    ? "Enter SMS template body. Use {{variable}} for dynamic content. (Max 160 characters recommended)"
                    : "Enter email template body. Use {{variable}} for dynamic content."}
                  required
                  disabled={saving}
                  maxLength={templateForm.channel === 'sms' ? 500 : undefined}
                />
                {templateForm.channel === 'sms' && (
                  <small>Character count: {templateForm.body.length}/500 (Recommended: 160 for single SMS)</small>
                )}
                {templateForm.channel === 'email' && (
